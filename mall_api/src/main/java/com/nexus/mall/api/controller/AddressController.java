...
import com.nexus.mall.util.MobileEmailUtils;
...
public class AddressController {
    ...
    @PostMapping("/list")
    public ServerResponse list(@NotBlank @RequestParam String userId) {
...
//        }
    ...
    private ServerResponse checkAddress(AddressBO addressBO) {
        ...
        if (StringUtils.isBlank(receiver)) {
            return ServerResponse.failed("收货人不能为空");
        }
        if (receiver.length() > 12) {
            return ServerResponse.failed("收货人姓名不能太长");
        }

        String mobile = addressBO.getMobile();
        if (StringUtils.isBlank(mobile)) {
            return ServerResponse.failed("收货人手机号不能为空");
        }
        if (mobile.length() != 11) {
            return ServerResponse.failed("收货人手机号长度不正确");
        }
        boolean isMobileOk = MobileEmailUtils.checkMobileIsOk(mobile);
        if (!isMobileOk) {
            return ServerResponse.failed("收货人手机号格式不正确");
        }
        ...
        if (StringUtils.isBlank(province) ||
            ...
            return ServerResponse.failed("收货地址信息不能为空");
        }
    ...
    @PostMapping("/update")
    public ServerResponse update(@Validated @RequestBody AddressBO addressBO) {
...
//        }
    ...
    public ServerResponse setDefault(
           ...
           @NotBlank @RequestParam("addressId") String addressId) {
